...
- 주제 레이더는 수집한 원본 신호를 보고서에 보존하되, 모델에는 검색어·채널별 최신 신호를 균형 있게 압축해 전달합니다.
- 기존 글 전체는 첫 분석에서 제목만 사용하고, 별도 중복 심사에서는 로컬 유사도 상위 글만 후보별로 전달합니다. 공식 웹 검색과 최종 중복 판정 단계는 유지합니다.
- 네이버 리서치는 원본 JSON과 함께 `.context.json` 요약본을 생성합니다. 글 작성에는 요약본을 먼저 사용하고, 필요한 사실이 없을 때만 원본을 확인합니다.
- OpenAI 분석과 중복 심사의 입력·출력·전체 토큰을 보고서에 기록해 회차별 품질과 사용량을 함께 비교합니다.
- 중복 심사는 기존 글뿐 아니라 같은 회차 후보끼리도 비교해 한 사업·행사·독자 행동을 제목 각도만 바꿔 반복 추천하지 않습니다.
- 정상 빌드에서는 `npm run build:quiet`를 사용해 성공 요약·경고·오류만 작업 컨텍스트에 남깁니다. 실패 시에는 진단에 필요한 마지막 로그를 출력합니다.
